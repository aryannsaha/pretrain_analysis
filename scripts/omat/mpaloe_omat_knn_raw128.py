#!/usr/bin/env python
"""Exact MP-ALOE -> OMAT24 top-10 neighbors in raw 128-d UMA embedding space.
...
Clone of scripts/omat/mad_omat_knn_raw128.py with the query set switched to the
single MP-ALOE matrix (909,792 rows; the dataset has no native splits); the
scan/audit/merge algorithm is unchanged, including the chunked reference-norm
accumulation that keeps the load phase inside 80 GB. This is a raw-128-d run
only -- no pc25 comparison is computed here.
"""
...
DEFAULT_RUN_ROOT = ROOT / "runs/omat_knn_probe/omat_knn_raw128_mpaloe"
DEFAULT_QUERIES = {
    "MPALOE_all": {
        "matrix": ROOT / "data/processed/mpaloe/uma_latents/all/mpaloe_all.npy",
        "sample_ids": ROOT / "data/processed/mpaloe/mpaloe_all_frames.tsv",
    },
}
JOINED_CSV_FIELDS = [
    "mpaloe_row", "mp_aloe_id", "formula", "natoms", "pbc", "rank",
...
def command_join(args):
    ...
    for name, spec in DEFAULT_QUERIES.items():
        ...
        sources = neighbor_source_frame(manifest, stops, indices.ravel())
        joined = pd.DataFrame({
            "mpaloe_row": np.repeat(np.arange(rows, dtype=np.int64), k),
            "mp_aloe_id": np.repeat(ids["mp_aloe_id"].to_numpy(object), k),
            "formula": np.repeat(ids["formula"].to_numpy(object), k),
